...
export const renderCharts = (
  ...
  const fs = window.innerWidth < 768 ? '18px' : '22px';
  ...
  if (state.currentMode === 'mortgage') {
    ...
    if (chart4El) {
      ...
      queueChartRender('chart4', t4, getBaseLayout('Equity Build-Up', 'Year', '$', state.isDark), PLOT_CONFIG);
    }
  }
  ...
  if (hasStrat) {
    ...
    if (chart9El) {
      ...
      }], getBaseLayout('Total Interest Cost', '', '$', state.isDark), PLOT_CONFIG);
    }
    ...
    if (chart12El) {
      ...
      }], getBaseLayout('Time to Pay Off', '', 'Years', state.isDark), PLOT_CONFIG);
    }
  }
};
