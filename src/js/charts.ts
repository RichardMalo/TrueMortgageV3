...
const flushRenderQueue = async () => {
  try {
    ...
    renderQueue.forEach(({ data, layout, config }, elementId) => {
      ...
      if (el && el.offsetWidth > 0 && el.offsetHeight > 0 && el.style.display !== 'none') {
...
const renderMonthlyPaymentCircle = (
  p1: { principal: number; interest: number; tax: number; ins: number; hoa: number; pmi: number; extra: number },
  totPITI: number,
  fs: string,
  tc: string
) => {
  ...
  if (!pieEl) return;
  const values = [p1.principal, p1.interest, p1.tax, p1.ins, p1.hoa, p1.pmi, p1.extra].filter(v => v > 0);
  const labels = ['Principal', 'Interest', 'Taxes', 'Insurance', 'HOA', 'PMI', 'Extra'].filter((_, i) => [p1.principal, p1.interest, p1.tax, p1.ins, p1.hoa, p1.pmi, p1.extra][i] > 0);
  
  queueChartRender('monthlyPaymentCircle', [{
    values,
    labels,
    type: 'pie',
    hole: 0.75,
    ...
    annotations: [{ text: `<b>Total/Period</b><br><span style="font-size: ${fs}; color: ${tc}">${formatCurrency(totPITI)}</span>`, showarrow: false }]
  }, PLOT_CONFIG);
};

const renderPaymentBreakdownCircle = (
  p1: { principal: number; interest: number },
  tc: string
) => {
  const breakEl = document.getElementById('paymentBreakdownCircle');
  if (!breakEl) return;
  queueChartRender('paymentBreakdownCircle', [{
    ...
    paper_bgcolor: 'rgba(0,0,0,0)',
    margin: { t: 0, b: 0, l: 0, r: 0 },
    annotations: [{ text: 'P & I Only', showarrow: false, font: { size: 14, color: tc, weight: 800 } }]
  }, PLOT_CONFIG);
};
...
const renderEquityBuildUpChart = (
  ...
  queueChartRender('chart4', t4, getBaseLayout('Equity Build-Up', 'Year', '$', isDark), PLOT_CONFIG);
};
...
const renderCumulativeOutflowChart = (
  ...
  queueChartRender('chart2', t2, getBaseLayout('Cumulative Outflow', 'Year', '$', isDark), PLOT_CONFIG);
};
...
const renderAnnualCashFlowChart = (
  ...
  queueChartRender('chart11', t11, Object.assign(getBaseLayout('Annual Cash Flow', 'Year', '$', isDark), { barmode: 'stack' }), PLOT_CONFIG);
};
...
const renderPaymentCompositionChart = (
  ...
  ], getBaseLayout('Payment Composition', 'Year', '$', isDark), PLOT_CONFIG);
};
...
const renderLifetimeBreakdownChart = (
  ...
  queueChartRender('chart', tTot, Object.assign(getBaseLayout('Lifetime Breakdown', '', '$', isDark), { barmode: 'stack' }), PLOT_CONFIG);
};
...
const renderLtvChart = (
  ...
  const lLTV: any = getBaseLayout('LTV (Loan To Value) & PMI (Private Mortgage Insurance) Drop', 'Year', 'LTV (%)', isDark);
  lLTV.yaxis.range = [0, Math.max(105, actualData.schedule[0]?.ltv || 100)];
  lLTV.shapes = [termLine, { type: 'line', x0: 0, y0: 80, x1: 1, y1: 80, xref: 'paper', yref: 'y', line: { color: CONFIG.colors.thresholdRed, width: 2, dash: 'dash' } }].filter(Boolean);
...
const renderInterestComparisonChart = (
  ...
  }], getBaseLayout('Total Interest Cost', '', '$', isDark), PLOT_CONFIG);
};
...
const renderPayoffTimeComparisonChart = (
  ...
  }], getBaseLayout('Time to Pay Off', '', 'Years', isDark), PLOT_CONFIG);
};
...
export const renderCharts = (
  ...
  const payoffX = (hasStrat && actualData.schedule.length > 0)
  ...
  renderDebtBalanceChart(state, baseData, actualData, compData, hasStrat, xKey, termLine, payoffLine);
  ...
  if (inputs.useOppCost) {
    ...
    renderOpportunityCostChart(state, baseData, actualData, compData, hasStrat, xKey, payoffLine, inputs);
  } else {
